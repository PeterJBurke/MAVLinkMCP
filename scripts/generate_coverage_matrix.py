...
The ``firmware_notes`` column (ArduPilot vs PX4 support caveats) is merged
from the hand-curated ``docs/firmware_notes.csv``, which records *observed*
SITL behavior only - rows without a recorded observation stay blank, never
guessed.
...
def main() -> None:
            ...
            rows.append(
                {
                    ...
                    "description": m["doc"],
                    "implemented_by_tools": implemented_in,
                    "status": status,
    ...
    lines = [
        ...
        "",
        "Paper Table 1 claimed ~40 of ~155 methods; introspection of the v1 code "
        f"found 41 of {client_total} client-side. The v2 branch is now at "
        f"{implemented} of {client_total} ({total} methods including server-side plugins).",
        "",
        "## Per-plugin counts",
        "",
    ...
    for pname in sorted(per_plugin):
        c = per_plugin[pname]
        used = "in use" if c["implemented"] else PLUGIN_PRIORITY.get(pname, "P3")
        lines.append(
    ...
    lines += [
        ...
        "## Notes",
        "",
        "- `firmware_notes` (ArduPilot vs PX4 caveats) is merged from the curated "
        "[`firmware_notes.csv`](firmware_notes.csv), which records observed SITL "
        "behavior only; untested rows stay blank, never guessed.",
