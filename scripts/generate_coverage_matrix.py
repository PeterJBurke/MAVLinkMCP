...
every public method of each plugin.  Then AST-parses the v1 server source
(``src/server/droneserver.py``) to map every registered MCP tool to the MavSDK
plugin method(s) it actually calls.
...
V1_SOURCE = REPO_ROOT / "src" / "server" / "droneserver.py"
...
def map_v1_usage(plugin_names: set[str]) -> tuple[dict, dict, set[str]]:
    """AST-parse the v1 server; return (tool_usage, infra_usage, all_tools).
    ...
    """
    tree = ast.parse(V1_SOURCE.read_text())
...
def main() -> None:
    ...
    tools_without_mavsdk = sorted(
        all_tools - {t for tools in tool_usage.values() for t in tools}
    )
    total = len(rows)
    ...
    lines = [
        ...
        f"`mavsdk=={mavsdk_version}` and `src/server/droneserver.py`.",
        ...
        f"- v1 MCP tools registered (`@mcp.tool()` in `src/server/droneserver.py`): "
        f"**{len(tool_names)}**",
        "",
    ...
    for pname in sorted(per_plugin):
        ...
        lines.append(
            f"| {pname} | {used} | {c['total']} | {c['implemented']} | "
            f"{c['missing']} | {c['candidate-N/A']} |"
        )
    lines += [
        "",
        "## v1 MCP tools",
