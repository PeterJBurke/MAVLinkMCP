...
for model in $MODELS; do
  ...
  timeout "$PER_MODEL_TIMEOUT" /root/.local/bin/uv run python scripts/run_llm_missions.py \
      ...
      2>&1 | grep -E -A2 "^model:|^protocol:|^price:|^budget:|PASS |FAIL |VOID|LINK |BUDGET|PROVIDER|ACCOMMODATION|spend:|spend on|capture:|degraded|ERROR|Error|Traceback|passed on"
  rc=${PIPESTATUS[0]}
  case "$rc" in
