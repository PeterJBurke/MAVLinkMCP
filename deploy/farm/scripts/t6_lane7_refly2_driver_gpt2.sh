#!/bin/bash
# Part 4 of the lane-7 T6 refly2 driver (2026-08-20): remaining gpt-5.2 rows
# (t2-t5) plus the newly-unblocked TG2 grok row, run AFTER the DEFECT C fix
# (commit 1720878) was deployed to the farm via rsync.
#
# DEFECT C fix note: run_llm_missions.py now writes a VOID row + attempts a
# safe landing on ANY mid-trial crash, signaled by exit code 5. Exit 5 is NOT
# a watchdog/precheck failure (unlike 9/10/11) - it is a legitimate
# VOID-not-scored outcome the fixed harness already handled safely. This
# driver treats it like any other non-halting rc: idle-check, log, continue.
set -u
...
echo "=================================================================="
echo "ALL REMAINING ROWS (gpt t2-t5 + TG2) COMPLETE $(date -u +%FT%TZ)"
echo "=================================================================="
exit 0
